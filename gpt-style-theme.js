/**
 * A ChatGPT-Inspired & OLED-App Theme for TypingMind
 *
 * This script themes the TypingMind interface to be visually similar to the
 * classic ChatGPT style, with OLED black backgrounds for dark mode.
 * 
...
(function () {
    ...
    const CONFIG = {
        ...
        fonts: { 
            primary: 'ui-sans-serif, -apple-system, system-ui, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"', 
            code: 'ui-monospace, SFMono-Regular, Menlo, Consolas, Liberation Mono, monospace', 
            weights: { light: 300, normal: 400, medium: 500, semibold: 600, bold: 700 }, 
            thought: { size: '12px', lineHeight: '20px', weight: 400 }, 
            sidebar: { size: '14px', lineHeight: '20px', weight: 400, family: 'ui-sans-serif, -apple-system, system-ui, "Segoe UI", Helvetica, "Apple Color Emoji", Arial, sans-serif, "Segoe UI Emoji", "Segoe UI Symbol"' } 
        },
    ...
    mainStyle.textContent = `
      ...
      html:not(.dark) [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] > div { font-family: ${CONFIG.fonts.primary} !important; font-size: 17px !important; line-height: 25px !important; color: ${light.text} !important; font-weight: ${CONFIG.fonts.weights.normal} !important; }
      html:not(.dark) [data-element-id="chat-space-middle-part"] .prose.max-w-full, html:not(.dark) [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] { font-family: ${CONFIG.fonts.primary} !important; font-size: 17px !important; line-height: 25px !important; color: ${light.text} !important; font-weight: ${CONFIG.fonts.weights.normal} !important; -webkit-font-smoothing: antialiased !important; -moz-osx-font-smoothing: grayscale !important; text-rendering: optimizeLegibility !important; letter-spacing: -0.005em !important; }
      html:not(.dark) [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] { margin-left: auto !important; margin-right: 0 !important; display: block !important; max-width: 95% !important; border-radius: ${CONFIG.borderRadius.large} !important; background-color: ${light.input.background} !important; color: ${light.text} !important; padding: 1rem !important; margin-bottom: ${CONFIG.spacing.small} !important; }
      ...
      /* ===== DARK MODE ===== */
      html.dark body, html.dark [data-element-id="chat-space-middle-part"] { background-color: ${dark.background} !important; }
      ...
      html.dark [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] > div { font-family: ${CONFIG.fonts.primary} !important; font-size: 17px !important; line-height: 25px !important; color: ${dark.text} !important; font-weight: ${CONFIG.fonts.weights.normal} !important; }
      html.dark [data-element-id="chat-space-middle-part"] .prose.max-w-full, html.dark [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] { font-family: ${CONFIG.fonts.primary} !important; font-size: 17px !important; line-height: 25px !important; color: ${dark.text} !important; font-weight: ${CONFIG.fonts.weights.normal} !important; -webkit-font-smoothing: antialiased !important; -moz-osx-font-smoothing: grayscale !important; text-rendering: optimizeLegibility !important; letter-spacing: -0.005em !important; }
      html.dark [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] { margin-left: auto !important; margin-right: 0 !important; display: block !important; max-width: 95% !important; border-radius: ${CONFIG.borderRadius.large} !important; background-color: ${dark.input.background} !important; color: ${dark.text} !important; padding: 1rem !important; margin-bottom: ${CONFIG.spacing.small} !important; }
      html.dark [data-element-id="chat-space-middle-part"] [data-element-id="user-message"], html.dark [data-element-id="chat-space-middle-part"] [data-element-id="user-message"] * { background-color: ${dark.input.background} !important; color: ${dark.text} !important; }
      ...
      /* ===== SHARED (both modes) & AI ALIGNMENT TWEAKS ===== */
      ...
      /* Hide ALL Avatars (User and AI) */
      [data-element-id="chat-space-middle-part"] [data-element-id="chat-avatar-container"] { display: none !important; }
    ...
    inputStyle.textContent = `
      ...
      /* 2. Kill the invisible fading gradient box above the input */
      html.dark .bg-gradient-to-t, 
      html.dark [class*="bg-gradient-"], 
      html.dark .scroll-indicator-gradient {
          background: transparent !important;
          background-image: none !important;
          background-color: transparent !important;
      }
      html.dark .scroll-indicator-gradient::before, 
      html.dark .scroll-indicator-gradient::after {
          display: none !important;
      }

      /* 3. Style the Input Bubble itself to be grey */
      html.dark [data-element-id="chat-space-end-part"] [role="presentation"] { 
          background-color: ${dark.input.background} !important; /* #303030 */
          border-radius: 1.5rem !important; 
          margin-bottom: ${CONFIG.spacing.medium}; 
      }
      /* Prevent child elements of the input bubble from overriding the grey */
      html.dark [data-element-id="chat-space-end-part"] [role="presentation"] *[class*="dark:bg-"] {
          background-color: transparent !important; 
      }
    ...
    function styleUserMessageEl(msgEl) {
        Utils.safe(() => {
            ...
            for (const child of msgEl.children) {
                if (hasAttachment && child.querySelector('.group\\/attachment')) { continue; }
    ...
    function initTheme() {
        ...
        const observeDomChanges = () => {
            const observer = new MutationObserver(mutations => {
                ...
                if (requiresVisualUpdate) { improveTextDisplay(); }
            });
            observer.observe(document.body, { childList: true, subtree: true });
        };
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => { improveTextDisplay(); observeDomChanges(); });
        } else {
            improveTextDisplay(); observeDomChanges();
        }
    }
